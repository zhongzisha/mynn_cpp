...
Cifar10Network_convg_t::Cifar10Network_convg_t(string net_name_, int gpu_id_) {
	net_name = net_name_;
	gpu_id = gpu_id_;
	curand_stream = NULL;
...
Cifar10Network_convg_t::~Cifar10Network_convg_t() {
	DestroyNet();
}

void Cifar10Network_convg_t::DestroyNet() {
...
void Cifar10Network_convg_t::BuildNet(int batch_size_, bool is_allocate_top_mem_, const string &net_params_file) {
	...
	// CURAND_CHECK( curandGenerateNormal(curand_generator, conv2g->biasBlob->data_gpu, conv2g->biasBlob->count(), (float)0.0f, (float)0.01f) );
	gpu_set(conv2g->biasBlob->count(), float(0.1f), conv2g->biasBlob->data_gpu);
	conv2g->Setup(mp1_top, conv2g_top, is_allocate_top_mem);
...
void Cifar10Network_convg_t::Forward(float *loss, float *accuracy) {
...
void Cifar10Network_convg_t::Backward() {
...
void Cifar10Network_convg_t::ForwardBackward(float *loss, float *accuracy) {
...
void Cifar10Network_convg_t::ComputeUpdateValueSingle(Blob_t *param_gradient_blob, Blob_t *param_blob_old,
...
void Cifar10Network_convg_t::ComputeUpdateValue(float lr_rate, float momentum, float weight_decay) {
	cudaSetDevice(gpu_id);
...
void Cifar10Network_convg_t::UpdateNet(float scale) {
	cudaSetDevice(gpu_id);
...
void Cifar10Network_convg_t::SaveNetParams(int epoch) {
	cudaSetDevice(gpu_id);
...
void Cifar10Network_convg_t::CopyNetParamsFrom(const Cifar10Network_convg_t *other) {
...
void Cifar10Network_convg_t::AddNetParamsDiffFrom(const Cifar10Network_convg_t *other) {
...
void Cifar10Network_convg_t::ClearNetParamsDiff() {
	cudaSetDevice(gpu_id);
