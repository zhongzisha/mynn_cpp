	...
	SimpleNet(string name_)
{
	...
	void BuildNet(int batch_size, int gpu_id_ = 0)
	{
		...
		layer_c1 = new CuDNNConvolutionLayer<float>(layer_param1);
		layer_c1->SetUp(c1_bottom_, c1_top_);
		printf("initialize the layers (done)..\n");
		...
		layer_c2 = new CuDNNConvolutionLayer<float>(layer_param4);
		...
		layer_c3 = new CuDNNConvolutionLayer<float>(layer_param41);
...
{
public:
	Blob<float> *batch_data;
	Blob<float> *batch_label;
	SimpleNet *net;
	int current_gpu_id;
	int net_gpu_id;
	float lr_rate;
...
void do_slave(void *data_)
{
	...
	CopyBlobData_gpu(data->batch_data,  data->current_gpu_id, data->net->c1_bottom_blob, data->net_gpu_id);
	CopyBlobData_gpu(data->batch_label,  data->current_gpu_id, data->net->label_blob, data->net_gpu_id);

	cudaSetDevice(data->net_gpu_id);
	printf("begin forward and backwrad ...\n");
	float trn_loss1 = data->net->ForwardBackward();
...
int main(int argc, char **argv)
{
	...
	ThreadData thread_data[gpus.size()];
	...
	for (int epoch = 0; epoch < max_epoch_num; ++epoch)
	{
		...
		else
		{
			for (int iter = 0; iter < (int)(50000/trn_batch_size); ++iter)
			{
				...
				for(int i = 0; i < gpus.size(); i++)
				{
					...
					thread_data[i].batch_data =slice_data_top_vec_[i];
					thread_data[i].batch_label =slice_label_top_vec_[i];
		...
		{
			lr_rate /= 10;
			trn_net->SaveNetParams(epoch);ret_count = pthread_attr_destroy(&pta);
		}
