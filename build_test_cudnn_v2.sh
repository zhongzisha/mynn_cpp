...
nvcc -m64 -ccbin=g++ -gencode arch=compute_50,code=sm_50 \
    -Xcompiler -fopenmp \
	-I${CUDA_ROOT}/include \
    ...
    -lmpi -lmpi++ \
    -L${CAFFE_ROOT}/build/lib -lcaffe -lcudnn \
	-lmatio \
    -o $EXE
...
# valgrind --tool=memcheck --leak-check=full --track-origins=yes --show-leak-kinds=all ./$EXE 
